...
export function normalizeAiBlogPayload(parsed) {
    ...
    const seoTitle = String(p.seo_title || p.seoTitle || '').trim().slice(0, 500);
    const metaDescription = String(p.meta_description || p.metaDescription || '').trim().slice(0, 2000);
    const keywords = String(p.keywords || '').trim();
...
Return ONLY valid JSON (no markdown fences) with this exact shape:
{
  ...
  "tags": ["tag1", "tag2"],
  "seo_title": "string (clean title optimized for search engines)",
  "meta_description": "string (short meta description for SEO under 160 chars)",
  "keywords": "string (comma-separated list of SEO keywords)",
  "hashtags": "string (space-separated list of hashtags e.g. #react #ai)",
  "status": "draft"
}
Rules:
